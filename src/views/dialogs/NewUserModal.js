...
const AdminNewUserModal = props => {
  const { isOpen, closeDialog, createNew, role, isEdit, currentUser } = props;
  const [open, setOpen] = React.useState(false);

  const [nameTxt, setNameTxt] = React.useState('');
  const [newRoleTxt, setnewRoleTxt] = React.useState('');
  ...
  React.useEffect(() => {
    ...
    if (isEdit && currentUser && currentUser.name) {
      setNameTxt(currentUser.name);
    }

    if (isEdit && currentUser && currentUser.role) {
      setnewRoleTxt(currentUser.role);
    }
  }, [currentUser]);
  ...
  return (
    <div>
      ...
      >
        ...
        <DialogActions>
          <Button
            onClick={isEdit === true ? editUser : handleSubmit}
            variant="contained"
            color="primary"
            style={{
